...
const Board = ({ params }: IBoardProps) => {
  ...
  const handleEditTicket = (id: number, containerId: number) => {
    const containerToEdit = data.find(
      (container: IBoardData) => container.id === containerId
    );
    const tiketToEdit = containerToEdit?.tickets.find(
  ...
  return (
    <section className="text-white bg-red-primary">
      <div className="flex h-screen p-4 gap-4 overflow-x-scroll">
          ...
          data.map((container: IBoardData) => {
            return (
              <div key={container.id}>
                ...
                >
                  {container.tickets.length ? (
                    container.tickets.map((ticket: ITicket) => {
                      return (
                        <Ticket
                          key={ticket.id!}
                          id={ticket.id!}
        ...
        <div className="bg-grey p-4 h-fit w-[400px] text-center cursor-pointer rounded-xl mb-4 hover:bg-black">
